  ...
  return (
    <motion.div className="space-y-8" variants={container} initial="hidden" animate="show">
      ...
      <motion.section variants={item}>
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-[var(--foreground)]">Recommended jobs</h2>
          <Link href="/jobs" className="text-sm font-medium text-[var(--gold)] hover:underline flex items-center gap-1">View all <ChevronRight className="h-4 w-4" /></Link>
        </div>
        <p className="mt-1 text-xs text-[var(--foreground-muted)]">Coaching roles that match your experience</p>
        <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {[
            { name: "Youth Head Coach", club: "FC North Academy", match: "Youth coach · Region" },
            { name: "Assistant Coach", club: "United South", match: "Assistant · Pro level" },
            { name: "Goalkeeper Coach", club: "City FC", match: "GK coach · Semi-pro" },
          ].map((rec) => (
            <Link key={rec.name} href="/jobs" className="glass-card group !rounded-xl p-4 flex items-center gap-4 hover:border-[var(--gold)]/30 transition-colors">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-[var(--foreground-muted)]">{rec.club} · {rec.match}</p>
              </div>
      ...
      <motion.section variants={item}>
        ...
        <p className="mt-1 text-xs text-[var(--foreground-muted)]">Track your coaching applications</p>
        <div className="glass-card mt-4 p-8 text-center">
          ...
          <Button variant="primary" size="sm" className="mt-3 rounded-full" asChild><Link href="/jobs">Find coaching jobs</Link></Button>
        </div>
      </motion.section>
      ...
      <motion.section variants={item} className="grid gap-4 sm:grid-cols-2">
        <Link href="/dashboard/messages" className="glass-card group !rounded-xl p-4 flex items-center gap-4 hover:border-[var(--gold)]/30 transition-colors">
          ...
          <div><p className="font-medium text-[var(--foreground)]">Messages</p><p className="text-xs text-[var(--foreground-muted)]">Chat with clubs</p></div>
        ...
        <Link href="/dashboard/favorites" className="glass-card group !rounded-xl p-4 flex items-center gap-4 hover:border-[var(--gold)]/30 transition-colors">
          ...
          <div><p className="font-medium text-[var(--foreground)]">Favorites</p><p className="text-xs text-[var(--foreground-muted)]">Saved clubs and jobs</p></div>
          <ChevronRight className="h-4 w-4 text-[var(--foreground-subtle)] group-hover:text-[var(--gold)] ml-auto transition-colors" />
        </Link>
      </motion.section>
    </motion.div>
